...
def main():
    ...
    with st.sidebar:
        ...
        art9_24013 = st.checkbox("Art. 9 Ley 24.013 (Registro tardío)", value=False, key="art9")
        fecha_registro = None
        if art9_24013:
            fecha_registro = st.date_input("Fecha de Registro", value=f_ingreso, min_value=f_ingreso, max_value=f_despido, format="DD/MM/YYYY")

        art10_24013 = st.checkbox("Art. 10 Ley 24.013 (Remuneración no registrada)", value=False, key="art10")
        remuneracion_no_registrada = 0.0
        fecha_inicio_art10 = None
        fecha_fin_art10 = None
        if art10_24013:
            remuneracion_no_registrada = st.number_input("Remuneración no registrada ($)", min_value=0.0, value=0.0, step=1000.0, format="%.2f")
            limitar_corte = st.checkbox("Período Específico (Modificar fechas)", value=False)
            if limitar_corte:
                col_i, col_f = st.columns(2)
                with col_i:
                    fecha_inicio_art10 = st.date_input("Inicio del Período", value=f_ingreso, format="DD/MM/YYYY")
                with col_f:
                    fecha_fin_art10 = st.date_input("Fin del Período", value=f_despido, format="DD/MM/YYYY")
    ...
    if sueldo > 0:
        try:
            ...
            if art9_24013 and fecha_registro:
                ...
                if meses_ui_9 > 0:
                    rubros.append((f"Multa Art. 9º Ley 24.013 ({meses_ui_9} meses)", (meses_ui_9 * sueldo) / 4))
            ...
            if art10_24013 and remuneracion_no_registrada > 0:
                ...
                if meses_ui_10 > 0:
                    rubros.append((f"Multa Art. 10 Ley 24.013 ({meses_ui_10} meses s/ ${remuneracion_no_registrada:,.2f})", (meses_ui_10 * remuneracion_no_registrada) / 4))
